        ...
        <div class="bg-white w-11/12 md:w-6/12 lg:w-4/12 m-6 rounded p-6">
            ...
            <form action="{{ route('token-verification') }}" method="post">
                @csrf
                 {{-- <div class="mb-4">
                    ...
                    <input type="email" placeholder="Enter email" name="email" id="email"
                        class="border-2 w-full p-4 bg-gray-200 rounded @error('email') border-red-500 @enderror"
                        value="">
                    @error('email')
                ...
                <div class="mb-4">
                    ...
                    <input type="password" placeholder="Enter confirm password " name="password_confirmation" id="password"
                        class="border-2 w-full bg-gray-100 p-4 rounded @error('password_confirmation') border-red-500 @enderror">
                </div>
                <div class="mb-6">
                    <button type="submit"
                        class="bg-blue-500 text-white px-4 py-3 rounded font-medium w-full capitalize">reset password</button>
                </div>
            </form>
        </div>
    </div>
